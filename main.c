...
void insert_in_quadtree(struct QuadTree* quadTree, void* leaf, int positionX, int positionY) {
    ...
    if (positionX > currentX) {
        if (positionY > currentY) {
            ((struct Node*)currentNode)->childrens[3] = leaf;
        } else {
            ((struct Node*)currentNode)->childrens[1] = leaf;
        }
    } else {
        if (positionY > currentY) {
            ((struct Node*)currentNode)->childrens[2] = leaf;
        } else {
            ((struct Node*)currentNode)->childrens[0] = leaf;
        }
    }
}
